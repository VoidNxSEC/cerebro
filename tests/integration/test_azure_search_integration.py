"""
Integration test suite for Azure AI Search backend.

Gated by CEREBRO_RUN_INTEGRATION=1 (skip if not set).
Requires: Azure AI Search instance with credentials in environment.
...
def provider():
    """
    Instantiate and return the Azure AI Search vector store provider.
    
    Skips the test if Azure AI Search is not available.
    """
    try:
        from cerebro.providers.vector_store_factory import build_vector_store_provider
        provider = build_vector_store_provider()
        
        # Verify it's actually azure_search
        if provider.backend_name != "azure_search":
            pytest.skip(
                f"Expected azure_search backend, got {provider.backend_name}. "
                "Set CEREBRO_VECTOR_STORE_PROVIDER=azure_search"
            )
        
        # Check health
        if not provider.health_check():
            pytest.skip("Azure AI Search backend not healthy")
        
        return provider
    except Exception as e:
        pytest.skip(f"Cannot instantiate azure_search provider: {e}")


# ============================================================================
# Test Suite: Azure AI Search Backend Contract
# ============================================================================


class TestAzureSearchIntegration:
    """Integration tests for Azure AI Search backend conformance."""

    def test_initialize_schema_creates_index(self, provider):
        """Index bootstrap creates index with vector settings."""
    ...
    def test_search_with_metadata_filters(
        ...
        """OData filters restrict search results correctly."""
        ...
        results = provider.search(
            query_embedding,
            top_k=10,
            filters={"source": "docs/intro.md"},
            namespace=integration_namespace,
        )
        
        # All results should have matching source
        for result in results:
            assert result.metadata.get("source") == "docs/intro.md"
    ...
    def test_namespace_isolation(
        ...
        ns1 = "inttest-az-ns1-" + "a" * 8
        ns2 = "inttest-az-ns2-" + "b" * 8
    ...
    def test_health_check_returns_true(self, provider):
        """Health check passes when Azure AI Search is accessible."""
    ...
    def test_get_backend_info_returns_metadata(self, provider):
        ...
        assert info["backend"] == "azure_search"
