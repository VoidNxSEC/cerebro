"""
Integration test suite for Weaviate backend.

Gated by CEREBRO_RUN_INTEGRATION=1 (skip if not set).
Requires: Weaviate running at connection URL in settings.
...
def provider():
    """
    Instantiate and return the Weaviate vector store provider.
    
    Skips the test if Weaviate is not available.
    """
    try:
        from cerebro.providers.vector_store_factory import build_vector_store_provider
        provider = build_vector_store_provider()
        
        # Verify it's actually weaviate
        if provider.backend_name != "weaviate":
            pytest.skip(
                f"Expected weaviate backend, got {provider.backend_name}. "
                "Set CEREBRO_VECTOR_STORE_PROVIDER=weaviate"
            )
        
        # Check health
        if not provider.health_check():
            pytest.skip("Weaviate backend not healthy")
        
        return provider
    except Exception as e:
        pytest.skip(f"Cannot instantiate weaviate provider: {e}")


# ============================================================================
# Test Suite: Weaviate Backend Contract
# ============================================================================


class TestWeaviateIntegration:
    """Integration tests for Weaviate backend conformance."""

    def test_initialize_schema_creates_class(self, provider):
        """Class bootstrap creates Weaviate class with vector settings."""
    ...
    def test_upsert_documents_ingests_batch(
        ...
        """Upsert a batch of documents into Weaviate."""
    ...
    def test_search_with_metadata_filters(
        ...
        """Property filters restrict search results correctly."""
        ...
        results = provider.search(
            query_embedding,
            top_k=10,
            filters={"title": "Cerebro Overview"},
            namespace=integration_namespace,
        )
        
        # All results should have matching title
        for result in results:
            assert result.metadata.get("title") == "Cerebro Overview"
    ...
    def test_namespace_isolation(
        ...
        ns1 = "inttest-wv-ns1-" + "a" * 8
        ns2 = "inttest-wv-ns2-" + "b" * 8
    ...
    def test_health_check_returns_true(self, provider):
        """Health check passes when Weaviate is accessible."""
