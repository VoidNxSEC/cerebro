"""
Integration test suite for OpenSearch backend.

Gated by CEREBRO_RUN_INTEGRATION=1 (skip if not set).
Requires: OpenSearch running at connection URL in settings.
...
def provider():
    """
    Instantiate and return the OpenSearch vector store provider.
    
    Skips the test if OpenSearch is not available.
    """
    try:
        from cerebro.providers.vector_store_factory import build_vector_store_provider
        provider = build_vector_store_provider()
        
        # Verify it's actually opensearch
        if provider.backend_name != "opensearch":
            pytest.skip(
                f"Expected opensearch backend, got {provider.backend_name}. "
                "Set CEREBRO_VECTOR_STORE_PROVIDER=opensearch"
            )
        
        # Check health
        if not provider.health_check():
            pytest.skip("OpenSearch backend not healthy")
        
        return provider
    except Exception as e:
        pytest.skip(f"Cannot instantiate opensearch provider: {e}")


# ============================================================================
# Test Suite: OpenSearch Backend Contract
# ============================================================================


class TestOpenSearchIntegration:
    """Integration tests for OpenSearch backend conformance."""
    ...
    def test_upsert_documents_ingests_batch(
        ...
        """Upsert a batch of documents into OpenSearch."""
    ...
    def test_namespace_isolation(
        ...
        ns1 = "inttest-os-ns1-" + "a" * 8
        ns2 = "inttest-os-ns2-" + "b" * 8
    ...
    def test_health_check_returns_true(self, provider):
        """Health check passes when OpenSearch is accessible."""
    ...
    def test_export_documents_with_embeddings(
        ...
        # Export - note: OpenSearch may return content-only without vectors
        try:
            ...
            # OpenSearch might not implement full export with embeddings
            pytest.skip("OpenSearch export_documents not implemented")
